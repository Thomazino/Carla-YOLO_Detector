...
def run_the_server():
    ...
    popen_spawn.PopenSpawn('CarlaUE4.exe -carla-server -benchmark -fps=28 -windowed -ResX=800 -ResY=600')


def darknet(message):
    os.chdir("C:/Yolo_v4/darknet/build/darknet/x64")
    process = popen_spawn.PopenSpawn('darknet.exe detector test data/objs.data \
                                    cfg/yolov4-objs.cfg backup/yolov4-objs_last.weights \
...
class CarlaGame(object):
    ...
    def _print_player_measurements(self, player_measurements, args):
        ...
        message = message.format(
            ...
            other_lane=100 * player_measurements.intersection_otherlane,
            offroad=100 * player_measurements.intersection_offroad)
        spd=player_measurements.forward_speed * 3.6
        

        
       

    def _traffic_light_recogniser(self, new, detected_colour_loop):
        global darknet_process
        global its_red
        global penalty
        global x_left,y_top,height,width
        global x_left2,y_top2,height2,width2
        carla_scene_exists = os.path.isfile("C:/Users/User/Desktop/carla_scene.jpg")
        if carla_scene_exists:
            carla_scene = (b"C:/Users/User/Desktop/carla_scene.jpg")
            darknet_process.send(carla_scene+b'\n')
            labels_file_exists = os.path.isfile("C:/Users/User/Desktop/carla_scene.txt")
            if labels_file_exists:
                labels_file = open("C:/Users/User/Desktop/carla_scene.txt","r")
                predicted_labels = labels_file.read()
                detected_colour_loop = ""
                if os.stat("C:/Users/User/Desktop/carla_scene.txt").st_size != 0:
                    print(predicted_labels)
                    x_left=float(predicted_labels[2:8])*800
                    y_top=float(predicted_labels[9:15])*600
                    width=float(predicted_labels[16:22])*800
                    height=float(predicted_labels[23:29])*600
                    #print(predicted_labels[2:8],predicted_labels[9:15],predicted_labels[16:22],predicted_labels[23:29])
                    #Check if any speed traffic sign has been detected
                    if len(predicted_labels)==30 or (int(predicted_labels[0])<3 and int(predicted_labels[30])<3):
                        x_left2,y_top2,height2,width2=0,0,0,0
                        prediction=int(predicted_labels[0])
                        if prediction==0:
                            its_red=True
                            detected_colour_loop="It's Red STOP"
                        elif prediction==1:
                            if its_red:
                                its_red=False
                            detected_colour_loop="It's Green GO"
                        elif prediction==2:
                            its_red=True
                            detected_colour_loop="It's Yellow BRAKE"
                        else:
                            if its_red:
                                its_red=False
                                penalty+=1
                            detected_colour_loop="Vehicle in FRONT"
                        new = True
                    else:
                        first_guess = int(predicted_labels[0])
                        second_guess = int(predicted_labels[30])
                        if first_guess!=second_guess:
                            x_left2=float(predicted_labels[2+30:8+30])*800
                            y_top2=float(predicted_labels[9+30:15+30])*600
                            width2=float(predicted_labels[16+30:22+30])*800
                            height2=float(predicted_labels[23+30:29+30])*600
                        prediction = first_guess if second_guess==3 else second_guess
                    ...
                    x_left,y_top,height,width=0,0,0,0
                    x_left2,y_top2,height2,width2=0,0,0,0
                    if its_red:
                        its_red=False
                        penalty+=1
                ...
                new=True
        return detected_colour_loop, new
    ...
    def _on_render(self, args):
        ...
        if self._main_image is not None:
            ...
            if (frame % 7 ==0):  
                    ...
                    a.save('C:/Users/User/Desktop/carla_scene.jpg')
                    #num+=1
                    detected_colour_loop, new = self._traffic_light_recogniser(new, detected_colour_loop)
   
            if new:
                detected_colour = detected_colour_loop
                
            
            if "GO" in detected_colour:
                r = 0
                g =255
                b = 0
            elif "Yellow" in detected_colour:
            ...
            text_current = basicfont.render('Penalty:'+str(penalty), True, (0,0,200))
